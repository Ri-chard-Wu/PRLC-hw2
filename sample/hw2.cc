...
// dummy function becase we dont need to know the ordit trap or 
// the object ID when we are calculating the surface normal
double map(vec3 p) {
    double dmy;  // dummy
    int dmy2;    // dummy2
        ...
        for (int j = 0; j < width; ++j) {
            ...
            for (int m = 0; m < AA; ++m) {
                for (int n = 0; n < AA; ++n) {
                    ...
                    vec3 cs = glm::normalize(glm::cross(cf, vec3(0., 1., 0.)));  // right (side) vector
                    ...
                    if (d < 0.) {        // miss (hit sky)
                        ...
                        col = pal(trap - .4, vec3(.5), vec3(.5), vec3(1.), vec3(.0, .1, .2));  // diffuse color
                        ...
                        double spe = glm::pow(glm::clamp(glm::dot(nr, hal), 0., 1.), gloss) * dif;  // self shadow
